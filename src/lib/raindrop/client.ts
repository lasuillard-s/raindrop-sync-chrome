...
export class Raindrop extends client.Raindrop {
	...
	async getAllRaindrops(
		...
		// NOTE: Raindrop API has a bug where if `sort` is omitted (undefined) and `page` is provided,
		// the backend ignores the `search` parameter and returns all items.
		// Passing `sort=''` or `sort='-created'` ensures `search` is properly applied by the API.
...
async function tryRefreshAccessToken(client: Raindrop, settings: SettingsStore): Promise<string> {
	try {
		...
		throw new Error(`Failed to refresh access token: ${err}`, { cause: err });
	}
}
