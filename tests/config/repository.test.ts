...
import { defaultBrowserProxy } from '~/lib/browser/proxy';
...
describe('BrowserSettingsRepository', () => {
	...
	it('returns default settings when load throws', async () => {
		const spyGet = vi.spyOn(defaultBrowserProxy.storage, 'get').mockImplementation(() => {
	...
	it('saves settings under the appSettings storage key', async () => {
		const spySet = vi.spyOn(defaultBrowserProxy.storage, 'set');
	...
	it('rethrows save errors', async () => {
		vi.spyOn(defaultBrowserProxy.storage, 'set').mockImplementation(() => {
			throw new Error('boom!');
		});
	...
	it('clears settings from the appSettings storage key', async () => {
		const spyRemove = vi.spyOn(defaultBrowserProxy.storage, 'remove');
		await repository.clear();
		expect(spyRemove).toHaveBeenCalledWith(BrowserSettingsRepository.STORAGE_KEY);
	});

	it('rethrows clear errors', async () => {
		vi.spyOn(defaultBrowserProxy.storage, 'remove').mockImplementation(() => {
			throw new Error('boom!');
		});
